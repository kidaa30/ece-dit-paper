...
\section{DIT in asynchronous systems}
  ...
  \label{sct:FPDITexist}
		...
		\[
			...
			T_j)) \; \forall i,j
		\]
		which means that there could be systems with no first periodic DIT.
		...
		of $a_2$ are 7 and 8. For a first periodic DIT to exist, one of those
		...
		\right.
		\]
		However those four conditions are false, so the system has no first
		periodic DIT (and no other DIT than $t=0$).

	\subsection{Simulation}

	We wanted to quantify how much systems has a first periodic DIT........
	...
	\end{itemize}
	and then computed the value of the first periodic DIT.\\

	We use the CDF because........

	The result are represented in Fig. \ref{fig:noFPDIT}
