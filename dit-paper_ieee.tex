...
%Thus, with a good description of the C-space, it is easy to check if a given vector of $C_i$ value (given e.g. when deploying the application on a specific platform) allows the system to be scheduled or not.
		...
		\subsubsection{Description using DBF}
			...
			constant (w.r.t. the $C_i$). Those equations are thus indeed linear.\\

			However, this description contains an infinite number of equations, most of
			...
			of constraints. In Section~\ref{sct:removeRedundancy} a method to reduce the size of this set of inequations is given.
	...
	\begin{remark} (from the definition):
		\begin{itemize}
			...
			first busy period.
			\item As $H$ is always a DIT in synchronous systems, we have $t_d \leq H$ in
			this case.
		\end{itemize}
	\end{remark}
	...
	the worst case [citation needed], it is sufficient for the DBF test to only consider intervals $[0, t]$ with $t$ being a deadline before the first DIT.\\
...
\[
	\begin{array}{|c|c|c|c|l}
		\text{Incomplete} & \text{Transitive} & \text{Stationary} & 2^{nd}
		\text{Stationary} & \cdots \\
