...
\section{DIT in asynchronous systems}
  ...
  \label{sct:FPDITexist}
	...
	\begin{itemize}
		\item Generating utilization values with the \texttt{UUniSort} algorithm described in \cite{bini2005measuring}, with parameters
		\begin{itemize}
...
To summarize, we first reduced the number of constraint from every possible interval $[t_1, t_2]$ to only the interval where $t_1$ is an arrival and $t_2$ is a deadline between $[t_d, t_d + H]$ if $t_d$ exists, or between $[O_{max}, O_{max} + 2H]$ otherwise. This gives us a first correct description of the C-SPACE which is not efficient. We then solve a linear integer problem per constraint which allow us to determine redundancy.
